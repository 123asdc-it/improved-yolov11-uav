"""
run_alpha_0p7.py — SA-NWD alpha=0.7 消融实验

hybrid loss = 0.7 * SA-NWD + 0.3 * CIoU
对比 alpha=0.5（当前最优）和 alpha=0.3，验证混合权重对结果的影响。

Architecture: nwd_p2（P2 head，与主消融一致）
Baseline for comparison: nwd_p2 alpha=0.5 → mAP50=0.9781

Server path: /root/drone_detection/
Output:      runs/ablation/nwd_p2_alpha07/
"""
...
patch_all_nwd(c_base=12.0, k=1.0, alpha=0.7, use_sa=True, nwd_min=0.0)
print('[alpha=0.7] loss = 0.7 * SA-NWD + 0.3 * CIoU, architecture: nwd_p2')

from ultralytics import YOLO

EXP_NAME = 'nwd_p2_alpha07'
...
result = {
    'name': 'alpha=0.7 (0.7*SA-NWD + 0.3*CIoU)',
    'exp': EXP_NAME,
    'alpha': 0.7,
